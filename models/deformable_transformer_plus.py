...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        # key and value from track mask
        # bs,h/8,w/8,257 -> bs,h/8,w/8,256
        self.conv_key = nn.Conv2d(257, 256, kernel_size=3, stride=1, padding=1)
        self.conv_value = nn.Conv2d(257, 256, kernel_size=3, stride=1, padding=1)
    ...
    # def forward(self, srcs, tgt, masks, pos_embeds, ref_pts=None):
        ...
        if track_mask.sum() != 0: 
            ...
            value = value.permute(0,2,3,1).reshape(bs, h*w, 256)
            memory_new_1 = torch.bmm(att, value)

            memory_features = [memory_new_1]
            spatial_index = h * w
            for lvl in range(1, self.num_feature_level):
                h, w = spatial_shapes[lvl]
                # [bs*t, c, h, w]
                memory_lvl = memory[:, spatial_index : spatial_index + h * w, :]
        ...
        if memory_new is not None:
            hs, inter_references, inter_samples = self.decoder(tgt, reference_points, memory_new,
        ...
        return hs, memory_features, init_reference_out, inter_references_out, None, None, inter_samples, track_memory
